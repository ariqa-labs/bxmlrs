...
impl<'bxml> AndroidManifest<'bxml> {
    ...
    pub fn parse(&mut self, arsc: Option<&Arsc>) -> Result<Vec<u8>, ParseError> {
        ...
        let (_, xml_chunk_header) = ChunkHeader::parse(self.binary_xml)
            ...
            match chunk_header.typ {
                ...
                ChunkType::XML_START_ELEMENT => {
                    ...
                    for _ in 0..xml_attr_ext.attribute_count {
                        ...
                        // attribute value
                        let mut attr_value: Option<String> =
                            attr.typed_value.as_string(&self.strings);
                        if let Some(arsc) = arsc {
                            let mut rec_count = 0;
                            while let Some(curr_attr_value) = &attr_value {
                                if curr_attr_value.starts_with("@res/0x") && rec_count < 5 {
                                    let res_id =
                                        u32::from_str_radix(&curr_attr_value[7..], 16).ok();
...
mod tests {
    ...
    fn test_xml_parser() -> Result<()> {
        let manifest_path = std::path::Path::new(
            "../data/xml/AndroidManifest-com.simplemobiletools.gallery.pro.xml",
        );
