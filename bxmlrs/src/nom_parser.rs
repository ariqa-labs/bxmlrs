...
impl TypeChunkHeader {
    ...
    pub(crate) fn parse(input: &[u8]) -> IResult<&[u8], TypeChunkHeader> {
        let (input, (header, id, flags, res1, entry_count, entries_start, config)) =
            tuple((
                ChunkHeader::parse,
                le_u8,
                le_u8,
...
impl ResValue {
    ...
    pub(crate) fn as_string(&self, strings: &[String]) -> Option<String> {
        match self.data_type {
            ...
            ResType::INT_BOOLEAN => {
                Some((if self.data != 0 { "true" } else { "false" }).to_string())
            }
...
pub mod parser {
    ...
    pub(crate) fn parse_string_pool_header(input: &[u8]) -> IResult<&[u8], StringPoolChunk> {
        map(
            ...
            |(header, string_count, style_count, flags, strings_start, styles_start)| {
                StringPoolChunk {
                    header,
                    string_count,
                    style_count,
